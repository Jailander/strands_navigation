...
class SimpleTopologicalNavServer(object):
        ...
        if self.current_node != msg.data :  #is there any change on this topic?
            ...
            if msg.data != 'none' :         #are we at a node?
                ...
                if self.navigation_activated :  #is the action server active?
                    ...
                    # if the robot reached an intermediate node and the next action is move base goal has been reached
...
#        goal.action_server=command
        ...
        self.NavClient = actionlib.SimpleActionClient(command, MoveBaseAction)
        print "sending goal to:"+ command
        print goal
   
#    client.wait_for_result(rospy.Duration.from_sec(5.0))
       
        self.NavClient.send_goal(goal)
        status=self.NavClient.get_state()
...
if __name__ == '__main__':
    ...
    server = SimpleTopologicalNavServer(rospy.get_name())
